...
<div class="container">
  ...
  <form action="<?= base_url("/updateMahasiswa/update/" . $mahasiswa["id"]); ?>" method="post" enctype="multipart/form-data">
    ...
    <div class="invalid-feedback">
        ...
        <!-- <div class="mb-3">
          <label for="prodi" class="form-label">Image</label>
          <input type="file" class="form-control <-?= ($validation && $validation->hasError('image')) ? 'is-invalid' : ''; ?>" id="image" name="image" placeholder="Inputan Prodi.." value="<?= $mahasiswa["image"]; ?>">
          <div class="invalid-feedback">
            <-?= ($validation && $validation->hasError('image')) ? 'is-invalid' : ''; ?>
          </div>
        </div> -->
